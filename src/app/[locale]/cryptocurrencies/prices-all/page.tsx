...
const PricesAll = () => {
	...
	const fetchCoinData = async () => {
		try {
			const res = await fetch(`/api/coinData?&limit=${loadNum}&page=${page}&sort=${sort}&desc=${desc}`);
			if (!res.ok) {
		...
		<div>
			...
			<div className="xl:mx-36 xl:my-20 md:mx-10 mx-3 my-20 text-white">
				<div className="flex sm:flex-row flex-col text-center sm:text-start justify-between items-center mx-5 sm:gap-5 gap-3">
					<div>
						<p className="md:text-3xl text-xl font-semibold">Coin Market<span className="bg-gradient-to-t from-blue via-blue to-lightblue text-transparent bg-clip-text ml-3">Today</span></p>
						<p className="text-description md:text-md text-sm">The global crypto market cap is $1.52T, a 5.06% decrease over the last day. </p>
					</div>
				</div>
			...
			<div className="text-white xl:mx-36 md:mx-10 mx-3">
				<div className="mt-20">
					<p className="text-white md:text-3xl text-xl md:text-start text-center font-semibold">Cryptocurrency Prices  <span className="bg-gradient-to-t from-blue via-blue to-lightblue text-transparent bg-clip-text">by Market Cap</span></p>
					<p className="text-description md:text-sm text-xs md:text-start text-center">The global crypto market cap is $1.52T, a 5.06% decrease over the last day. </p>
				</div>
					...
					<div className="lg:flex grid sm:grid-cols-4 grid-cols-2 lg:gap-1 gap-3 my-3">
						{
							pricesALL.dropdown.map((item: PriceAllDropdown) =>
								<Button size={"filters"} key={item.value} value={item.value} className="bg-metricbasic flex justify-between hover:bg-description text-white">{item.text}<ChevronDown /></Button>)
						}
						{/* <DropdownMenu>
					...
					<div className="flex justify-end items-center gap-5">
						<Drawer direction="right">
							<DrawerTrigger asChild>
								<Button size={"filters"}><GeneralFilter />More Filters</Button>
							</DrawerTrigger>
							<DrawerContent>
						...
						<div className="flex items-center gap-1 border-2 border-description text-description rounded-md">
							<div className="hover:bg-description hover:text-white p-1 m-1 rounded-md"><List className="2xl:w-5 2xl:h-5 md:w-4 md:h-4" /></div>
							<div className="hover:bg-description hover:text-white p-1 m-1 rounded-md"><LayoutGrid className="2xl:w-5 2xl:h-5 md:w-4 md:h-4" /></div>
						</div>
					</div>
				</div>
				<div className="h-auto bg-metricbasic rounded-lg p-8 my-3">
					<div className="lg:flex justify-between">
						...
						<div className="flex gap-2 items-center border-2 border-description xl:px-3 xl:py-2 px-2 py-1 rounded-lg">
							...
							<input type="text" placeholder="search" className="w-full bg-metricbasic outline-none" onChange={(e) => setSearch(e.target.value)} />
						</div>
					</div>
					<Table>
						<TableHeader>
						...
						<TableBody>
							{coinData && coinData
								.filter(coin => coin.categories && coin.categories.includes(filter))
								.filter(coin => coin.name && coin.name.toLowerCase().includes(search))
								.map((coin) => (
